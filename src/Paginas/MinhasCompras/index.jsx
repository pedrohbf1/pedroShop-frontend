...
import CadaCompra from "../../Componentes/CadaCompra";

const PaginaMinhasCompras = () => {
    ...
    useEffect(() => {
        axios.post("http://localhost:3000/buscar-minhas-compras", {}, {
          headers: {
        ...
        .then((response) => {
          ...
          const produtosUnicos = response.data.reduce((acc, produto) => {
            ...
            const existente = acc.find(item => item.codigoVenda === produto.codigoVenda);
          ...
          setTodosProdutos(produtosUnicos);
        ...
        <>
            ...
            <Centralizador>
                {todosProdutos && todosProdutos.map((produto, index) => {
                    return <CadaCompra key={index} compra={produto} />
                })}
            </Centralizador>
        </>
    )
...
export default PaginaMinhasCompras
