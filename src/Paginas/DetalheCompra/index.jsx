...
import DetalheCompranfs from "../../Componentes/DetalheDeCompranfs";

const PaginaDetalheCompra = () => {
    ...
    const [todaCompra, setTodaCompra] = useState("")

    useEffect(() => {
        axios.get(`http://localhost:3000/minhas-compras/${id}`)
        .then((r) => {
            setTodaCompra(r.data)
        })
        .catch((erro) => {
            console.log(erro)
        })
    ...
    return (
        <>
            ...
            <Centralizador>
                {todaCompra && todaCompra.map((compra, index) => {
                    return <DetalheCompranfs key={index} compra={compra} />
                })}
            </Centralizador>
        </>
    )
...
export default PaginaDetalheCompra
